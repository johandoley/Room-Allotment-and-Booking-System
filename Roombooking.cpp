...
top:
    ...
    while (n <= q) {
        ...
        if (chair > 40)
            cout << "Only 40 chairs are available in the room!";
        else {
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                if (strcmp(hotel[n].chair[i][j], "Empty") != 0) {
                    // Uncomment if needed: cout<<endl<<" The chair no."<<(n-1)<<"is reserved for : "<<hotel[a].chair[i][j]<<".";
                }
            }
        }
        break;
...
void reservation::avail() {
    for (int n = 0; n < q; n++) {
        ...
        cout << "Room no: \t" << hotel[n].room_number << "\nHost: \t" << hotel[n].host << "\t\tStart time: \t" << hotel[n].start << "\tEnd  Time: \t"
             << hotel[n].send << "\nFrom: \t\t" << hotel[n].from << "\t\tTo: \t\t\t" << hotel[n].to << "\n";
        design('#');
        design('_');
    }
}
